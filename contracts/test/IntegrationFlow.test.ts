...
describe("Integration: deposit → borrow → swap → repay → withdraw (MC-074)", () => {
  async function deployFullStack() {
    ...
    return { vault, pool, router, composer, registry, oracle, token, tokenB, owner, executor, execContract: execContractDeployed };
  }

  it("can open position with supply only (no borrow, no swap)", async () => {
    const { vault, pool, composer, registry, token, owner } =
    ...
    const [eSupply] = await userClient
  ...
  it("can submit and cancel a swap intent", async () => {
    ...
    const intentId = await router.connect(user).submitSwapIntent.staticCall(
    ...
    await router.connect(user).submitSwapIntent(
      await token.getAddress(),
      await tokenB.getAddress(),
      amount,
      amount,
  ...
  it("can execute intent with executor contract", async () => {
    ...
    await execContract.connect(owner).approveToken(
      await tokenB.getAddress(),
      await router.getAddress(),
      outputAmount,
    );
    ...
    await execContract.connect(owner).executeIntent(
      await router.getAddress(),
      intentId,
      outputAmount,
    );
