...
/// @title  PriceOracle
/// @notice Token → USD-1e18-WAD prices backed by Pyth Network, with per-token
///         risk parameters (collateral factor, liquidation threshold, stale
///         threshold).
contract PriceOracle {
    ...
    /// @notice Bound on the absolute Pyth `expo` after WAD adjustment.
    int256 public constant MAX_PYTH_EXP = 38;

    uint256 public immutable DEFAULT_STALE_THRESHOLD;
    IPyth public immutable PYTH;

    /// @notice Pyth price-feed id per token. Zero = unsupported.
    mapping(address => bytes32) public priceId;
    /// @notice Per-token staleness window (seconds). Zero ⇒ DEFAULT_STALE_THRESHOLD.
    mapping(address => uint64) public staleThreshold;
    /// @notice Cached ERC-20 decimals. Zero ⇒ WAD_DECIMALS.
    ...
    /// @notice Configure a Pyth price source for `token`. `priceId_ == 0` unsets it.
    function setSource(
        address token,
        bytes32 priceId_,
        uint8 decimals_,
    ...
    /// @notice Configure per-token risk parameters; `liqThresholdBps >= ltvBps`.
    ...
    /// @notice Push fresh signed Pyth VAA updates into the on-chain Pyth cache.
    ...
    /// @return feeAmount Native fee (wei) required to call `updatePriceFeeds`.
    ...
    /// @notice Latest USD price for `token` in 1e18 scale.
    /// @return priceWad  Price in USD * 1e18.
    /// @return updatedAt Unix timestamp of the underlying Pyth round.
    function getPriceUsd(address token) public view returns (uint256 priceWad, uint64 updatedAt) {
        ...
        // Reject wide-confidence aggregates: conf must be ≤ 1% of price.
        if (p.conf < 1) revert UncertainPrice();
        if (uint256(p.conf) * BPS_DEN > absAnswer * 100) revert UncertainPrice();

        // Normalise to 1e18 WAD: priceWad = absAnswer * 10^(18 + expo).
    ...
    /// @notice Convert raw-units `amount` of `token` to USD-1e18-WAD.
    ...
    /// @notice Convert USD-1e18-WAD `usdWad` to raw-units of `token`.
    ...
    /// @notice Owner-only sweep for any ETH stuck in this contract.
