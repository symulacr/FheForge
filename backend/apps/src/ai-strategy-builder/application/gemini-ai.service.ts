...
function getBestTemplate(riskLevel?: string, maxLoops?: number) {
  ...
  if (maxLoops !== undefined)
    filtered = filtered.filter((t) => t.loopCount <= maxLoops);
...
function adaptTemplate(
  ...
  return template.steps.map((s, i) => {
    ...
    if (step.tokenOut)
      step.tokenOut = { ...step.tokenOut, amount: Math.round(amount * 0.7) };
    return step;
  });
}
...
export class GeminiAiService {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      if (i > 0) {
        ...
        if (valid[prev] && !valid[prev].includes(step.type))
          warnings.push(
            `Step ${i + 1}: Unusual sequence ${prev} → ${step.type}`,
          );
      }
    }
    if (!steps.some((s) => s.type === 'SUPPLY'))
      warnings.push('Strategy has no supply steps');
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        if (!Array.isArray(steps) || !steps.length)
          throw new Error('Invalid strategy steps');
  ...
  private mapGeminiError(error: unknown): never {
    ...
    if (msg.includes('timeout'))
      throw new GeminiApiException('Request timeout.', 408);
...
Return ONLY a JSON array of steps. Format: [{"step":1,"type":"SUPPLY","agent":"COFHE","tokenIn":{"assetId":"...","symbol":"...","amount":100}}]`;
  ...
  private extractInputToken(input: string): {
    ...
    const m = input.match(
      /(\d+(?:\.\d+)?)\s*(WETH|USDT|USDC|WBTC|DAI|ARB|LINK|UNI|AAVE|ETH)/i,
    );
  ...
  private isMaximizeYieldRequest(input: string): boolean {
    ...
    return (
      l.includes('maximize yield') ||
      l.includes('maximum yield') ||
      l.includes('best yield')
    );
  }
  ...
  private async generateMaximizeYieldStrategy(
    ...
    if (!best)
      return this.generateStrategySteps(
        userIntent,
        additionalContext,
        tokenAmount,
      );
    const initialToken =
      this.extractInitialToken(additionalContext) || inputToken;
