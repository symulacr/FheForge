...
export const isNodeConfigured = (node: Node<DefiNodeData>) => {
	...
	const operationType =
		(config as { operationType?: string; type?: string } | undefined)?.operationType ||
	...
	return Boolean(config?.tokenInId && config?.tokenOutId && config?.amount != null);
};
...
export const validateAddNode = ({
	...
	if (lastNode && !isNodeConfigured(lastNode) && selectedNode?.id !== lastNode.id) {
		return {
			valid: false,
	...
	if (lastNode) {
		...
		const connectionResult = validateConnection(lastOperationType, newOperationType);
