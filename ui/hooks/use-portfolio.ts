...
function isPositionIdArray(data: unknown): data is PositionId[] {
	return (
		Array.isArray(data) &&
		data.every((item): item is PositionId => typeof item === "string" && item.startsWith("0x"))
	);
}
...
export function usePortfolio(account?: Address) {
	...
	const userPositions: PositionId[] = isPositionIdArray(userPositionsData) ? userPositionsData : [];
	...
	const getPositionMeta = (_positionId: PositionId) => {
		const index = userPositions.indexOf(_positionId);
