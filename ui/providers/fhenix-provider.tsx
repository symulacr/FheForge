...
import { ARBITRUM_SEPOLIA_CHAIN_ID, DEFAULT_RETRY_COUNT, QUERY_STALE_TIME } from "@/lib/constants";
...
async function getClient() {
	...
	if (!cofheClientSingleton) {
		...
		cofheClientSingleton = factory(createCofheConfig({ supportedChains: [arbSepolia] }));
	}
	...
	return (
		<CofheClientContext.Provider value={client}>
			<CofheStateContext.Provider value={state}>
				<CofheProvider
					cofheClient={client as unknown as Parameters<typeof CofheProvider>[0]["cofheClient"]}
				>
					{children}
				</CofheProvider>
