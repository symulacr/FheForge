...
export default function ConfigPanel({ node, nodes, onSave, onClose }: Props) {
	...
	const resolvedType = useMemo<DefiOperationType>(() => {
		const raw =
			node?.data?.action?.type || node?.data?.action?.operation_type || node?.data?.action?.name;
	...
	const selectedPair = useMemo(() => {
		if (requiresTokenOut) {
			return pairs.find(
				(p: DefiPair) => p?.token_in?.id === tokenIn && p?.token_out?.id === tokenOut,
			);
		}
	...
	useEffect(() => {
		...
		if (config?.tokenInPairId || config?.tokenInId) {
			...
			if (prevOutputTokenId) {
				let pair = pairs.find((p: DefiPair) => p?.token_in?.asset_id === prevOutputTokenId);
	...
	const correctTokenOut = useCallback(
		(newTokenIn: string, currentTokenOut: string) => {
			...
			const validPair = pairs.find(
				(p: DefiPair) => p?.token_in?.id === newTokenIn && p?.token_out?.id === currentTokenOut,
			);

			if (validPair) return currentTokenOut;

			const firstPair = pairs.find((p: DefiPair) => p?.token_in?.id === newTokenIn);
	...
	const handleSubmit = async () => {
		...
		const tokenInMeta = selectedPair?.token_in || tokenInOptions.find((t) => t.id === tokenIn);

		const tokenOutMeta = selectedPair?.token_out || tokenOutOptions.find((t) => t.id === tokenOut);
		...
		const payload: SaveConfigPayload = {
			...
			tokenOutId: requiresTokenOut ? tokenOutMeta?.asset_id || tokenOut || "" : "",
	...
	const renderEstimate = () => {
		...
		return (
			<EstimateCard estimate={estimate} operationType={resolvedType} selectedPair={selectedPair} />
		);
	};
        ...
        "
			>
				...
				<div className="flex-1 overflow-y-auto px-6 py-6 space-y-6 custom-scroll">
					{pairsLoading && (
						<div className="space-y-4" role="status" aria-label="Loading action requirements">
									...
									>
										<div className="flex items-center gap-3">
											...
											<span className="text-sm font-medium">
												{tokenInOptions.find((t) => t.id === tokenIn)?.name || "Select Token"}
											</span>
										</div>
										<div
                  ...
                  "
									...
									<div className="relative">
										...
										>
											<div className="flex items-center gap-3">
												...
												<span className="text-sm font-medium">
													{tokenOutOptions.find((t) => t.id === tokenOut)?.name || "Select Token"}
												</span>
											</div>
											<div
							...
							{estimating && (
								<div role="status" aria-label="Estimating output" className="space-y-2">
