...
import { TX_HASH_SHOW_LIMIT, ARBITRUM_SEPOLIA_EXPLORER } from "@/lib/constants";
...
export const MyActivityTable = () => {
  ...
  const handleReExecute = async (row: MyActivityRow) => {
    ...
    try {
      ...
      displayToast(
        "success",
        "Simulation loaded successfully! Ready to re-execute.",
      );
    } catch (error) {
      const message =
        error instanceof Error ? error.message : "Re-execution failed";
