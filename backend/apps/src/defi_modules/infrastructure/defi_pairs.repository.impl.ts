...
function mapPair(row: DefiPairRow): DefiPair {
  return new DefiPair(
    row.id,
    '',
    row.token_in_id ?? undefined,
    row.token_out_id ?? undefined,
  );
}
...
export class DefiPairsRepositoryImpl implements DefiPairsRepository {
  ...
  public async findByTokenInId(tokenInId: string): Promise<DefiPair[]> {
    ...
    if (error) {
      throw new Error(
        `Failed to fetch DefiPairs by token_in: ${error.message}`,
      );
    }
  ...
  public async findByTokenOutId(tokenOutId: string): Promise<DefiPair[]> {
    ...
    if (error) {
      throw new Error(
        `Failed to fetch DefiPairs by token_out: ${error.message}`,
      );
    }
  ...
  public async findByTokenPair(
    ...
    if (error) {
      throw new Error(
        `Failed to fetch DefiPairs by token pair: ${error.message}`,
      );
    }
