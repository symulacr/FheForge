...
async function bootstrap() {
	...
	const allowedOrigins = allowedOriginsRaw
		...
		: nodeEnv === "production"
			? (Logger.error("ALLOWED_ORIGINS env var is required in production", "Bootstrap"),
				process.exit(1))
			: DEV_ORIGINS;
	...
	if (nodeEnv === "development") {
		...
		swaggerConfig.addServer("https://fheforge-api-staging.up.railway.app", "Staging server");
	} else {
		swaggerConfig.addServer("https://fheforge-api-production.up.railway.app", "Production server");
	}

	SwaggerModule.setup("api/docs", app, SwaggerModule.createDocument(app, swaggerConfig.build()));
