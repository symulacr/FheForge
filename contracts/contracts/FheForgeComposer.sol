...
interface IStrategyVault {
    ...
    function openPosition(address token, uint256 amount, euint128 encAmount, uint256 strategyId, address user) external;

    function addCollateral(address collateralToken, uint256 amount, InEuint64 calldata encAmount) external;

    function addCollateral(address collateralToken, uint256 amount, InEuint64 calldata encAmount, address user) external;

    function addCollateral(address collateralToken, uint256 amount, euint128 encAmount, address user) external;

    function closePosition(uint256 collateralAmount, InEuint128 calldata encCollateralAmount) external;
...
interface ILendingPool {
    ...
    function supplyToLending(address token, uint256 amount, InEuint64 calldata encAmount, address user) external;

    function supplyToLending(address token, uint256 amount, euint64 encAmount, address user) external;
    ...
    function borrowFromLending(address token, uint256 amount, InEuint64 calldata encAmount, address user) external;

    function borrowFromLending(address token, uint256 amount, euint64 encAmount, address user) external;

    function repay(address token, uint256 amount, InEuint64 calldata encAmount) external;

    function repayBorrow(address token, uint256 amount, InEuint64 calldata encAmount, address user) external;

    function repayBorrow(address token, uint256 amount, euint64 encAmount, address user) external;
...
contract FheForgeComposer is ReentrancyGuard, Pausable {
    ...
    event StrategyRebalanced(
        address indexed user, uint256 indexed addAmount, uint256 indexed repayAmount, uint256 newBorrowAmount
    );
    event Paused();
    event Unpaused();
    ...
    constructor(address registry_, address vault_, address pool_, address router_) {
        if (registry_ == address(0) || vault_ == address(0) || pool_ == address(0) || router_ == address(0)) {
            revert ZeroAddress();
        }
    ...
    function openLeveragedStrategy(OpenStrategyParams calldata p, OpenStrategyEncrypted calldata e)
        external
        nonReentrant
        whenNotPaused
        returns (uint256 strategyId, bytes32 intentId)
    {
        ...
        emit LeveragedStrategyOpened(msg.sender, strategyId, intentId, p.poolSupplyAmount, p.poolBorrowAmount);
    }

    function _resolveStrategyId(OpenStrategyParams calldata p) internal returns (uint256) {
        if (p.strategyId == 0) {
            return REGISTRY.registerStrategy(p.strategyName, p.workflowHash, p.apyTarget, p.loopCount);
        }
        return p.strategyId;
    }
    ...
    function _supplyToPool(OpenStrategyParams calldata p, OpenStrategyEncrypted calldata e, uint256 permitCovered)
        internal
    {
        if (p.poolSupplyAmount == 0) return;
        uint256 needsPull = p.poolSupplyAmount > permitCovered ? p.poolSupplyAmount - permitCovered : 0;
        if (needsPull > 0) {
            IERC20(p.collateralToken).safeTransferFrom(_msgSender(), address(this), needsPull);
        }
    ...
    function _borrowFromPool(OpenStrategyParams calldata p, OpenStrategyEncrypted calldata e) internal {
    ...
    function _submitSwap(OpenStrategyParams calldata p, OpenStrategyEncrypted calldata /* e */) internal returns (bytes32) {
        if (p.swapTokenOut == address(0)) return bytes32(0);
        return
            ROUTER.submitSwapIntent(
                p.collateralToken, p.swapTokenOut, p.swapAmountIn, p.swapMinOut, p.swapDeadlineOffset
            );
    }
    ...
    function rebalance(RebalanceParams calldata p, RebalanceEncrypted calldata e) external nonReentrant whenNotPaused {
        ...
        if (p.addCollateralAmount > 0) {
            uint256 needsPull = p.addCollateralAmount > collateralPulled ? p.addCollateralAmount - collateralPulled : 0;
            if (needsPull > 0) {
                IERC20(p.collateralToken).safeTransferFrom(_msgSender(), address(this), needsPull);
            }
    ...
    function _pullViaPermit2(address token, Permit2Authorization calldata auth) internal returns (uint256 pulled) {
