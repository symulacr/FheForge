...
export class AiStrategyBuilderService {
  ...
  async buildStrategy(dto: BuildStrategyDto) {
    const steps = await this.geminiAi.generateStrategySteps(
      dto.userIntent, dto.additionalContext, dto.tokenAmount,
    );
    const validation = this.geminiAi.validateSteps(steps);
    let aiAnalysis: { riskFactors: string[]; recommendations: string[] } | undefined;
    try { aiAnalysis = await this.geminiAi.analyzeStrategyRisk(steps); } catch { /* non-critical */ }
    const riskLevel = aiAnalysis
      ? (aiAnalysis.riskFactors.length === 0 ? 'LOW' : aiAnalysis.riskFactors.length >= 3 ? 'HIGH' : 'MEDIUM')
      : (steps.filter(s => s.type === 'BORROW').length / steps.length > 0.4 ? 'HIGH' : 'MEDIUM');
    ...
    return {
      ...
      metadata: { totalSteps: steps.length, estimatedGas, riskLevel, aiGenerated: true },
