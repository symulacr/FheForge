...
export function StrategyOverview({ strategy, simulateData }: StrategyOverviewProps) {
	const safeAgents =
		simulateData?.steps?.map((s) => s.agent).filter(Boolean) || strategy?.agents || [];
		...
		<div className="space-y-6">
			...
			<div className="glass p-6 space-y-5">
				<div className="flex items-center gap-3 ">
					...
					<h3 className="text-xl font-semibold text-foreground">Strategy Description</h3>
				</div>

				{simulateData ? (
					<p className="text-foreground/80 leading-relaxed text-[15px]">{safeDescription}</p>
				) : (
				...
				{safeAgents.length > 0 && (
					<div className="pt-4 border-t border-border/50">
						<div className="flex items-center gap-2 mb-3">
							...
							<span className="text-sm font-semibold text-foreground/90">Executed by Agents</span>
						</div>
