...
export default function StrategyReviewClient() {
  ...
  const handleExecute = async () => {
    ...
    const supplyStep = steps.find(
      (s) => s.type === "SUPPLY" || s.action === "SUPPLY",
    );
    if (!supplyStep) {
    ...
    const borrowStep = steps.find(
      (s) => s.type === "BORROW" || s.action === "BORROW",
    );
    ...
    if (storedStrategyId <= 0) {
      displayToast(
        "error",
        "No valid strategy ID found. Please generate a strategy first.",
      );
      return;
    }
            ...
            <div className="border border-border bg-card p-6">
              ...
              <div className="space-y-2">
                <div className="text-sm text-muted">
                  <span className="font-medium">Starting Token:</span>{" "}
                  {selectedToken}
                </div>
                <div className="bg-secondary p-3 text-sm text-foreground">
                  {prompt}
                </div>
              </div>
            </div>
            ...
            <div className="border border-border bg-card p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span
                    className={`text-sm ${getRiskLevelColor(safeMetadata.riskLevel)}`}
                  >
