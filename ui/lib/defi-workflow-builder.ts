...
export const buildWorkflowJson = (nodes: Node<DefiNodeData>[]) => {
	...
	const steps = nodes.map((node, index) => {
		...
		if (index === 0 && config.tokenInId === "") {
			throw new Error(`Node "${node.data?.action?.name ?? node.id}" has an empty tokenInId`);
		}
		...
		return {
			step: stepNumber++,
			type: (node.data.action?.name ?? node.id).toUpperCase().replace(/\s+/g, "_"),
	...
	const swapCount = nodes.filter((node) => node.data?.action?.operation_type === "SWAP").length;
