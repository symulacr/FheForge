...
export class ActivityRepositoryImplement implements ActivityRepository {
	...
	async findAll(): Promise<Activity[]> {
		...
		return (data || []).map((row: OnChainEventRow) => this.mapEventToEntity(row));
	}

	async findByFilter(filters: { strategyId?: string; userAddress?: string }): Promise<Activity[]> {
		try {
			let query = this.supabase.getClient().from("on_chain_events").select("*");
			...
			if (filters.userAddress) {
				...
				query = query.filter("data->>user", "eq", filters.userAddress.toLowerCase());
			}
	...
	async findPaginated({
		...
		try {
			...
			if (userAddress) {
				query = query.filter("data->>user", "eq", userAddress.toLowerCase());
			}
			...
			return {
				data: (data ?? []).map((row: OnChainEventRow) => this.mapEventToEntity(row)),
				total: count ?? 0,
			};
		} catch (err) {
			this.logger.warn(`findPaginated error: ${err instanceof Error ? err.message : String(err)}`);
			return { data: [], total: 0 };
		}
	}
	...
	async save(activity: Activity): Promise<void> {
		...
		const { error } = await this.supabase.getClient().from("activities").upsert(payload);
