import { type CofheClient, Encryptable } from "@cofhe/sdk";
...
describe("Comprehensive FheForge Integration & Flow Test Suite", () => {
	...
	describe("2. StrategyRegistry Timelock & Archiving", () => {
		it("reverts if empty, duplicate, or unregistered names are parsed", async () => {
			...
			await expect(registry.registerStrategy("", hash)).to.be.revertedWithCustomError(
				registry,
				"EmptyName",
			);
			await expect(registry.registerStrategy("a".repeat(257), hash)).to.be.revertedWithCustomError(
				registry,
				"NameTooLong",
			);
			await expect(
				registry.registerStrategy("Strategy-A", ethers.ZeroHash),
			).to.be.revertedWithCustomError(registry, "ZeroWorkflowHash");

			await registry.registerStrategy("Strategy-A", hash);
			await expect(registry.registerStrategy("Strategy-A", hash)).to.be.revertedWithCustomError(
				registry,
	...
	describe("3. StrategyVault Position Lifecycle & Native ETH", () => {
		it("handles partial deposit, collateral additions, and full position closure cleanly", async () => {
			...
			const [eCollateral] = await testerClient
				.encryptInputs([Encryptable.uint128(collateralAmt)])
				.execute();
			...
			// Full close
			const [eRemaining] = await testerClient
				.encryptInputs([Encryptable.uint128(remaining)])
				.execute();
	...
	describe("4. LendingPool Supply/Borrow & Native Wrapping Flow", () => {
		it("supports supply, checkLtvAndBorrow, repay, and withdraw gates", async () => {
			...
			const [eSupply] = await testerClient
				.encryptInputs([Encryptable.uint128(supplyAmt)])
				.execute();
			await pool.connect(tester).shield(await usdc.getAddress(), supplyAmt, eSupply);

			// Borrow with dynamic LTV verification (70%)
			const [eBorrow] = await testerClient
				.encryptInputs([Encryptable.uint128(borrowAmt)])
				.execute();
			await pool
				.connect(tester)
				.borrowWithLtvCheck(
			...
			await expect(
				pool.connect(tester).partialUnshield(await usdc.getAddress(), supplyAmt, eWd),
		...
		it("integrates with native ETH wrappers", async () => {
			...
			const [eSupplyEth] = await testerClient
				.encryptInputs([Encryptable.uint128(ethAmt)])
				.execute();
			...
			// Native withdraw
			const [eWithdrawEth] = await testerClient
				.encryptInputs([Encryptable.uint128(ethAmt)])
				.execute();
	...
	describe("7. FheForgeComposer Compound Leverage Flow", () => {
		it("successfully opens and manages leveraged portfolios in a single call", async () => {
			...
			const [eCol, eSup, eBor] = await testerClient
				.encryptInputs([
