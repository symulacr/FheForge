...
export const createStrategyWorkflow = async (payload: CreateStrategyRequest) => {
...
export const getStrategiesByOwner = async (ownerId: string, signal?: AbortSignal) => {
...
export const getRequiredActionData = async (actionId: string) => {
	const res = await api.get(`/defi-modules/actions/required?action_id=${actionId}`);
	return res.data;
};
