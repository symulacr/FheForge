...
export class EventIndexerService implements OnModuleInit, OnModuleDestroy {
	...
	async onModuleInit(): Promise<void> {
		...
		if (blockGap > 128) {
			...
			this.logger.warn(`Block gap ${blockGap} > 128, clamping checkpoint to block ${clamped}`);
	...
	private async persistEvents(events: IndexedEvent[]): Promise<void> {
		try {
			...
			const { error } = await this.supabaseService.getClient().from("on_chain_events").insert(rows);
