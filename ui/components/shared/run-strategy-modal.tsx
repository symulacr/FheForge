...
export default function RunStrategyModal({ open, onOpenChange, strategyId, onSimulated }: Props) {
	...
	const handleRun = async () => {
		...
		try {
			...
			const msg = err instanceof Error ? err.message : "Failed to simulate strategy";
			...
			<DialogContent className="sm:max-w-[440px] bg-card border-border p-0 overflow-hidden">
				<div className="p-8">
					<DialogHeader className="flex flex-col items-center text-center space-y-3 mb-6">
						...
						<p className="text-sm text-muted-foreground">Enter the amount to simulate execution</p>
					</DialogHeader>
