...
export class GeminiAiService {
	...
	async generateStrategySteps(
		...
		while (retryCount <= maxRetries) {
			try {
				...
				this.logger.error(`Gemini API error (attempt ${retryCount + 1}/${maxRetries + 1}):`, error);
				lastError = error;
	...
	private extractInputTokenFromIntent(input: string): {
		...
		const initialTokenMatch = input.match(/initial\s+token\s+is\s+(WETH|USDT|USDC)/i);
		const withTokenMatch = input.match(/with\s+(\d+(?:\.\d+)?)\s*(WETH|USDT|USDC)/i);
	...
	private extractInitialTokenFromContext(additionalContext?: string): string | undefined {
		...
		const initialTokenMatch = additionalContext.match(/initial\s+token\s+is\s+(WETH|USDT|USDC)/i);
	...
	private needsInitialSwap(
		...
		const firstStepMatch = userIntent.match(/(?:1\.\s*)?(?:supply|lend|swap|borrow)\s+(\w+)/i);
	...
	private filterDeadTypes(steps: StrategyStepResponseDto[]): StrategyStepResponseDto[] {
