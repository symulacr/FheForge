	...
	async function permitGrant() {
		try {
			...
			const {
				...
				createWalletClient: createWalletClient2,
				http: http2,
				custom: custom2,
			} = await import("viem");
	...
	try {
		...
		initErrors.wallet =
			err instanceof BridgeError
				...
				: new BridgeError(
						"WALLET_INIT_FAILED",
						err.message || "Failed to initialize wallet adapter",
						"wallet",
						true,
					);
	...
	try {
		...
		initErrors.api =
			err instanceof BridgeError
				...
				: new BridgeError(
						"API_INIT_FAILED",
						err.message || "Failed to initialize API adapter",
						"api",
						false,
					);
		return buildPartialResult(adapters, initErrors, mergedConfig);
	...
	try {
		...
		initErrors.fhe =
			err instanceof BridgeError
				...
				: new BridgeError(
						"FHE_INIT_FAILED",
						err.message || "Failed to initialize FHE adapter",
						"cofhe",
						true,
					);
		return buildPartialResult(adapters, initErrors, mergedConfig);
		...
		initErrors.contract =
			err instanceof BridgeError
				...
				: new BridgeError(
						...
						err.message || "Failed to initialize contract adapter",
						"contract",
						false,
					);
		return buildPartialResult(adapters, initErrors, mergedConfig);
