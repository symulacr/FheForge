...
|---|---|---|---|---|---|---|---|---|
| 1 | A.3 | stress | GAP | StrategyVault.sol | closePosition L210-236 | partial close strands collateral; deletes all state regardless of `collateralAmount` | A | [x] |
| 2 | A.5b2 | stress | GAP | LendingPool.sol | (state invariant) | reserve(token) < Σ plainSupply after any borrow | A | [x] |
| 3 | A.5c | stress | GAP | LendingPool.sol | withdraw L220-252 | passes plain check but ERC-20 transfer reverts when reserves were drained by a borrow | A | [x] |
| 4 | C.3 | stress | GAP | StrategyRegistry.sol | Strategy.active L19 | flag set true at register, never updated | E | [x] |
| 5 | C.4 | stress | GAP | StrategyRegistry.sol | setVault L84-90 | vaultAddress is one-shot; cannot rotate | E | [x] |
| 6 | C.5 | stress | GAP | SwapRouter.sol | EXECUTOR L35 | immutable; no rotation path | A | [x] |
| 7 | F.1 | stress | GAP | (all) | — | no batched ops anywhere | C | OPEN |
| 8 | F.2 | stress | GAP | StrategyVault.sol | openPosition / addCollateral | users approve+call vault directly; no Composer | C | [x] |
| 9 | F.3 | stress | GAP | StrategyVault, LendingPool | (no payable) | no native ETH | C | [x] |
| 10 | F.4 | stress | GAP | StrategyVault.sol | closePosition | no partial unwind / scaleDown | A | [x] |
...
| 12 | F.6 | stress | GAP | LendingPool.sol | withdraw L220 | not gated by outstanding borrow | A | [x] |
...
| 25 | Q.5 | stress | GAP | LendingPool.sol | checkLtvAndBorrow | reserve drainable with own collateral at 100/100 LTV | A | [x] |
...
| 30 | W.1 | stress | GAP | (deps) | — | no Permit2 / ERC-1363 | C | [x] |
...
| 36 | X.1–X.4 | stress | GAP | all | (no pause) | no circuit breakers | A | [x] |
| 37 | X.5 | stress | GAP | all | — | no upgrade proxy | DEFERRED | UUPS migration is significant; post-MVP |
| 38 | X.6 | stress | GAP | LendingPool.sol | — | no emergency withdraw | A | [x] [Wave 2] |
| 39 | X.7 | stress | GAP | StrategyRegistry.sol | setVault | no timelock on admin | E | [x] |
...
| 42 | Y.2 | stress | GAP | LendingPool.sol | — | no oracle | E | [x] |
| 43 | Y.3 | stress | GAP | LendingPool.sol | — | no per-token LTV | E | [x] |
| 44 | Y.4 | stress | GAP | StrategyRegistry.sol | encryptedTvls | TVL not normalised | DEFERRED | depends on Y.2 oracle wiring |
| 45 | Y.5 | stress | GAP | LendingPool.sol | — | no liquidation threshold | DEFERRED | [x] |
| 46 | Z.1–Z.5 | stress | GAP | governance | — | no DAO, no token, no human ID, no KYC, no cross-chain | DEFERRED | full DAO is roadmap-level |
| 47 | AA.1 | stress | WARN | LendingPool.sol | checkLtvAndBorrow | accepts ltvNum=0 | A | [x] |
...
| 52 | AA.6 | stress | WARN | StrategyRegistry.sol | decrementTvl | full handle decrement | A | [x] [Wave 2] |
| 53 | A.4 | stress | WARN | StrategyVault.sol | addCollateral | uses ZeroAddress for token mismatch | A | [x] |
| 54 | B.5 | stress | WARN | StrategyRegistry.sol | registerStrategy | accepts empty name | E | [x] |
| 55 | B.7a | stress | WARN | SwapRouter.sol | submitSwapIntent | accepts deadlineOffset=0 | A | [x] |
| 56 | B.8 | stress | WARN | SwapRouter.sol | submitSwapIntent | no upper bound on deadlineOffset | A | [x] |
...
| 58 | Q.3 | stress | WARN | StrategyRegistry.sol | registerStrategy | accepts ZeroHash workflow | E | [x] |
| 59 | Q.4 | stress | WARN | StrategyRegistry.sol | registerStrategy | duplicate names allowed | E | OPEN |
| 60 | B.6 (LIMIT) | stress | LIMIT | StrategyRegistry.sol | registerStrategy | 16KB name = 12M gas DoS | E | [x] |
...
|---|---|---|---|---|---|---|---|
| 61 | SH-1 | SwapRouter.sol | 52 | gas-indexed-events | `IntentSubmitted.tokenIn` not indexed | D | [x] |
...
| 64 | SH-4 | SwapRouter.sol | 63 | gas-indexed-events | `IntentExecuted.outputAmount` not indexed | D | [x] |
| 65 | SH-5 | SwapRouter.sol | 94 | gas-increment-by-one | `nonces[msg.sender]++` use `++variable` | D | OPEN |
| 66 | SH-6 | StrategyVault.sol | 78 | gas-indexed-events | `CollateralAdded.amount` not indexed | D | [x] |
| 67 | SH-7 | StrategyVault.sol | 83 | gas-indexed-events | `PositionClosed.collateralAmount` not indexed | D | [x] |
| 68 | SH-8 | StrategyRegistry.sol | 14 | gas-struct-packing | `Strategy` struct packing | D | OPEN |
| 69 | SH-9 | LendingPool.sol | 41 | gas-indexed-events | `Supplied.amount` not indexed | D | [x] |
| 70 | SH-10 | LendingPool.sol | 57 | gas-indexed-events | `Repaid.amount` not indexed | D | [x] |
| 71 | SH-11 | LendingPool.sol | 62 | gas-indexed-events | `Withdrawn.amount` not indexed | D | [x] |
| 72 | SH-12 | LendingPool.sol | 129 | gas-strict-inequalities | use strict `<` instead of `<=` | D | [x] |
...
|---|---|---|---|---|---|---|---|
| 73 | SL-1 | StrategyVault.sol | 109-163 | reentrancy-benign | state writes after FHE precompile (intentional; FHE.asEuintN is cryptographic, not callback-capable) | A | [x] DOCUMENT [Wave 1] |
| 74 | SL-2 | SwapRouter.sol | 87-130 | reentrancy-benign | same | A | [x] [Wave 1] |
| 75 | SL-3 | SwapRouter.sol | 87-130 | reentrancy-events | event emitted after FHE precompile | A | [x] [Wave 1] |
