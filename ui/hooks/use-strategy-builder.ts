...
import { createDefiEdge, createDefiNode, markLastNode } from "@/lib/defi-node-factory";
import { buildWorkflowJson } from "@/lib/defi-workflow-builder";
import { useUser } from "@/providers/user-provider";
import { submitStrategy } from "@/services/defi-strategy-builder";
import type { Action, CreateStrategyPayload, Module } from "@/types/defi";
...
const getEstimateAmountOut = (est: StrategyResult | undefined, fallbackAmount?: number) => {
	return (
		est?.amount_out ??
		est?.output_amount ??
		est?.result_amount ??
...
const getEstimateOutputAssetId = (est: StrategyResult | undefined, payload: SaveConfigPayload) => {
	return (
		est?.token_out_id ||
		est?.output_token_id ||
		est?.result_asset_id ||
...
const getEstimateOutputSymbol = (est: StrategyResult | undefined, payload: SaveConfigPayload) => {
	return (
...
export function useDefiBuilder() {
	...
	const [selectedNode, setSelectedNode] = useState<Node<DefiNodeData> | null>(null);
	...
	const onConnect = useCallback(
		(params: Edge | Connection) => {
			setEdges((eds) => {
				const alreadyConnected = eds.some((edge) => edge.source === params.source);
	...
	const saveConfig = useCallback(
		async (payload: SaveConfigPayload) => {
			...
			const actionName = payload.operationType || action?.name?.toUpperCase?.() || "SWAP";

			try {
				const finalEstimate = payload.estimate ?? null;
				const estimateForCalc = finalEstimate ?? undefined;

				const rawTokenOutId = getEstimateOutputAssetId(estimateForCalc, payload);
	...
	const createStrategy = useCallback(
		async (name: string) => {
			...
			try {
				...
				const supplyNode = nodes.find((n) => n.data?.config?.operationType === "SUPPLY");
				...
				if (!supplyNode && !addCollateralNode) {
					...
					const acTxHash = await addCollateral(collateralToken, amount);
					if (acTxHash) {
						await waitForTransactionReceipt(
							{ chainId } as unknown as Parameters<typeof waitForTransactionReceipt>[0],
							{ hash: acTxHash, pollingInterval: 2000 } as unknown as Parameters<
					...
					if (strategyId === 0) {
						...
						const opTxHash = await openPosition(collateralToken, collateralEth, BigInt(strategyId));
						if (opTxHash) {
							await waitForTransactionReceipt(
								{ chainId } as unknown as Parameters<typeof waitForTransactionReceipt>[0],
								{ hash: opTxHash, pollingInterval: 2000 } as unknown as Parameters<
									typeof waitForTransactionReceipt
								>[1],
							);
						}
		...
		[user, nodes, router, openPosition, addCollateral, chainId],
	);
