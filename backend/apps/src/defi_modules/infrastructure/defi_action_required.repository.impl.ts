...
export class DefiActionRequiredRepositoryImplement implements DefiActionRequiredRepository {
  ...
  async findRequiredActionsByActionId(
    actionId: string,
    ...
    const filteredData = (defiModules as ModuleWithActions[] | null)?.map(
      (item) => ({
        ...item,
        defi_module_actions:
          requiredActionIds.size > 0
            ? item.defi_module_actions.filter((defiAction) =>
                requiredActionIds.has(defiAction.id),
              )
