...
function markCoverage(contract: string, func: string, status: CoverageEntry["status"], gap?: string) {
...
class TestingContext {
  ...
  public static getContext(): TestingContext {
    if (!this.instance) this.instance = new TestingContext();
...
async function main() {
  ...
  try {
    ...
    if (positions.length > 0) {
      ...
      if (dep0 > 0n) {
        ...
        await (await helper.closePosition(enc[0], await vault.getAddress(), positionId, dep0)).wait();
      }
    }
  } catch { /* ignore */ }
  ...
  {
    const ok = await staticCallRevert(registry, "registerStrategy", ["", ethers.zeroPadValue("0x01", 32)], "EmptyName");
  ...
  {
    const ok = await staticCallRevert(registry, "registerStrategy", ["x".repeat(257), ethers.zeroPadValue("0x01", 32)], "NameTooLong");
    record("1.2", ok ? "PASS" : "FAIL", "Oversize name rejected", ok ? "NameTooLong" : lastRevertError);
  }

  // 1.3 Register — zero workflow hash
  {
    const ok = await staticCallRevert(registry, "registerStrategy", [`Z-${ts}`, ethers.ZeroHash], "ZeroWorkflowHash");
    record("1.3", ok ? "PASS" : "FAIL", "Zero workflow hash rejected", ok ? "ZeroWorkflowHash" : lastRevertError);
  }
  ...
  if (testStrategyId) {
    const ok = await staticCallRevert(registry, "registerStrategy", [`str-${ts}`, ethers.zeroPadValue("0x02", 32)], "StrategyAlreadyExists");
    record("1.4", ok ? "PASS" : "FAIL", "Duplicate (creator,name) rejected", ok ? "StrategyAlreadyExists" : lastRevertError);
  }
  ...
  if (testStrategyId) {
    try {
      ...
      const ok = !isActive;
      record("1.5a", ok ? "PASS" : "FAIL", "setActive(false) archives strategy", `active=${isActive}`, { tx: txOff.hash });
      ...
      record("1.5b", meta2[4] === true ? "PASS" : "FAIL", "setActive(true) restores", `active=${meta2[4]}`, { tx: txOn.hash });
    } catch (e: any) {
  ...
  try {
    ...
    record("1.6a", ok ? "PASS" : "WARN", `proposeVault sets ${vaultDelay}s timelock`, `earliest=${earliest}`, { tx: txProp.hash });
  ...
  // 1.7 acceptVault — before timelock elapses
  {
    ...
    record("1.7", ok ? "PASS" : "WARN", "acceptVault before timelock reverts", ok ? "TimelockNotElapsed" : lastRevertError);
  ...
  if (testStrategyId) {
    const ok = await staticCallRevert(registry, "incrementTvl", [testStrategyId, ethers.zeroPadValue("0x64", 32)], "OnlyVault");
    record("1.8a", ok ? "PASS" : "WARN", "incrementTvl non-vault reverts", ok ? "OnlyVault" : lastRevertError);
    markCoverage("StrategyRegistry", "incrementTvl", "static_call", "1.8a");
  }
  if (testStrategyId) {
    const ok = await staticCallRevert(registry, "decrementTvl", [testStrategyId, ethers.zeroPadValue("0x64", 32)], "OnlyVault");
    record("1.8b", ok ? "PASS" : "WARN", "decrementTvl non-vault reverts", ok ? "OnlyVault" : lastRevertError);
  ...
  try {
    ...
    const txOpen = await withRetry(async () => {
      return await helper.openPosition(eCollateral, await vault.getAddress(), USDC, collateralAmt, 1n, tester.address);
    }, "openPosition");
    const rOpen = await txOpen.wait();
    const vaultAddress = await vault.getAddress();
    const vLog = rOpen!.logs.find((log: any) =>
      log.address?.toLowerCase() === vaultAddress.toLowerCase() && log.fragment?.name === "PositionOpened"
    );
  ...
  if (positionId) {
    ...
    } catch { record("2.2a", "WARN", "getPositionMeta view", "failed"); }
    try {
    ...
    } catch { record("2.2b", "WARN", "getPositionMeta view", "failed"); }
  }

  // 2.3 addCollateral — wrong token
  if (positionId) {
    try {
      const [eAdd] = await cofhe.encryptInputs([Encryptable.uint128(1n)]).execute();
  ...
  if (positionId) {
    try {
      ...
      const txClose = await helper.closePosition(eClose, await vault.getAddress(), positionId, closeAmt);
      await txClose.wait();
      const remaining = (await vault.getDepositedAmount(positionId)) as bigint;
      record("2.4", remaining === collateralAmt - closeAmt ? "PASS" : "FAIL", "Partial close keeps state", `remaining=${remaining}`, { tx: txClose.hash });
  ...
  if (positionId) {
    try {
      ...
      const txFull = await helper.closePosition(eFull, await vault.getAddress(), positionId, remaining);
      await txFull.wait();
      const has = (await vault.getUserPositions(tester.address)).includes(positionId);
      record("2.5", has ? "FAIL" : "PASS", "Full close clears state", `hasPosition=${has}`, { tx: txFull.hash });
    } catch (e: any) {
  ...
  try {
    ...
    const tx = await helper.openPosition(eCol, await vault.getAddress(), USDC, collateralAmt, 1n, tester.address);
    const r = await tx.wait();
    const vaultAddress = await vault.getAddress();
    const vLog = r!.logs.find((log: any) =>
      log.address?.toLowerCase() === vaultAddress.toLowerCase() && log.fragment?.name === "PositionOpened"
    );
    positionId = vLog?.args?.[0];
    record("2.6", "PASS", "Position re-opened for pool tests", `Id=${positionId}`, { tx: tx.hash });
  } catch { record("2.6", "WARN", "Re-open for pool tests", "failed"); }
  ...
  try {
    ...
    const tx10 = await helper.openPosition(e10, await vault.getAddress(), USDC, freshAmt, 1n, tester.address);
    const r10 = await tx10.wait();
    const vAddr = await vault.getAddress();
    const posLog10 = r10!.logs.find((log: any) =>
      log.address?.toLowerCase() === vAddr.toLowerCase() && log.fragment?.name === "PositionOpened"
    );
    const newPosId = posLog10?.args?.[0];
    if (newPosId) {
      const isValidBytes32 = typeof newPosId === "string" && newPosId.startsWith("0x") && newPosId.length === 66;
      record("2.10", isValidBytes32 ? "PASS" : "WARN", "PositionOpened encoding roundtrip", `positionId=${newPosId} len=${newPosId.length}`, { tx: tx10.hash });
    } else {
      record("2.10", "WARN", "PositionOpened encoding roundtrip", "no positionId in event");
    }
    if (newPosId) { positionId = newPosId; }
  } catch (e: any) {
  ...
  try {
    ...
    record("3.2", "PASS", "borrowWithLtvCheck", `borrow=${ethers.formatUnits(plainBorrow, 6)} USDC`);
  ...
  // 3.3 borrowWithLtvCheck — ltvNum=0 (should revert)
  {
    ...
    const ok = await staticCallRevert(pool, "borrowWithLtvCheck", [USDC, USDC, 1n, eZero, 0, 100], "LtvNumeratorZero");
    record("3.3", ok ? "PASS" : "FAIL", "ltvNum=0 rejected", ok ? "LtvNumeratorZero" : lastRevertError);
  ...
  // 3.6 borrowWithOracle — full path
  {
    ...
    try {
      ...
      record("3.6b", "PASS", "borrowWithOracle uses oracle price", `gas=${rBo!.gasUsed}`, { tx: txBo.hash });
  ...
  // 3.8 isLiquidatable — oracle health check
  {
    ...
    try {
      ...
      record("3.8a", !healthy ? "PASS" : "FAIL", "healthy position not liquidatable", `isLiquidatable=${healthy}`);
    } catch (e: any) {
      record("3.8a", "WARN", "isLiquidatable healthy failed", decodeRevert(e));
    }
    // Unhealthy position (massive debt far beyond LTV)
    try {
      const unhealthy = await pool.isLiquidatable(tester.address, USDC, USDC, collateralAmt, borrowAmt * 100n);
      record("3.8b", unhealthy ? "PASS" : "FAIL", "unhealthy position liquidatable", `isLiquidatable=${unhealthy}`);
    } catch (e: any) {
  ...
  const okFl = await staticCallRevert(pool, "flashLoan", [USDC, 1n, tester.address, "0x"], "TokenMismatch");
  record("3.10", okFl ? "PASS" : "INFO", "flashLoan reverts without state", okFl ? "TokenMismatch" : lastRevertError);
  ...
  try {
    await pool.liquidateWithProof.staticCall(USDC, tester.address, ethers.ZeroHash, "0x", ethers.ZeroHash, "0x");
  ...
  try {
    ...
    record("3.12", !liq ? "PASS" : "FAIL", "isLiquidatable no borrow", !liq ? "false" : "unexpected true");
  } catch (e: any) {
  ...
  {
    const ok = await staticCallRevert(pool, "liquidateWithProof", [USDC, tester.address, ethers.ZeroHash, "0x", ethers.ZeroHash, "0x"], "CannotSelfLiquidate");
    record("3.13", ok ? "PASS" : "INFO", "liquidateWithProof access control", ok ? "CannotSelfLiquidate" : lastRevertError);
  }
  ...
  {
    const ok1 = await staticCallRevert(pool, "flashLoan", [USDC, 0n, tester.address, "0x"], "ZeroAmount");
    const err1 = lastRevertError;
    const ok2 = await staticCallRevert(pool, "flashLoan", [USDC, 1n, ethers.ZeroAddress, "0x"], "ZeroAddress");
    const err2 = lastRevertError;
    const detail = ok1 && ok2 ? "both reverted as expected" : `zeroAmount=${ok1 ? "\u2713" : err1} zeroAddr=${ok2 ? "\u2713" : err2}`;
  ...
  {
    const ok = await staticCallRevert(pool, "flashLoan", [USDC, 1n, deployer.address, "0x"], "OnlyVault");
    record("3.17", ok ? "PASS" : "INFO", "flashLoan integrator role", ok ? "OnlyVault" : lastRevertError);
  }
  ...
  // 3.18 — flash loan callback pattern documentation (INFO)
  {
    ...
    record("3.18", "INFO", "flashLoan callback pattern",
  ...
  {
    const ok = await staticCallRevert(router, "submitSwapIntent", [USDC, WETH, 1n, 1n, 0n], "DeadlineTooShort");
    record("4.1", ok ? "PASS" : "FAIL", "deadlineOffset=0 rejected", ok ? "DeadlineTooShort" : lastRevertError);
  ...
  // 4.2 submitSwapIntent — deadlineOffset >> MAX (too long)
  {
    ...
    const ok = await staticCallRevert(router, "submitSwapIntent", [USDC, WETH, 1n, 1n, far], "DeadlineTooLong");
    record("4.2", ok ? "PASS" : "FAIL", "deadlineOffset > MAX rejected", ok ? "DeadlineTooLong" : lastRevertError);
  ...
  try {
    ...
    record("4.5", ok ? "PASS" : "WARN", `proposeExecutor ${execDelay}s timelock`, `earliest=${earliest}`, { tx: txP.hash });
  ...
  // acceptExecutor before timelock
  {
    ...
    record("4.5b", ok ? "PASS" : "WARN", "acceptExecutor before timelock", ok ? "TimelockNotElapsed" : lastRevertError);
  ...
  try {
    ...
    record("4.6", "INFO", "swapViaUniswapV3Single (needs active swap state)", "reverts as expected");
  }
  ...
  {
    const ok = await staticCallRevert(router, "swapViaUniswapV3Single", [USDC, WETH, 0n, 1n], "ZeroAmountIn");
    record("4.11", ok ? "PASS" : "FAIL", "swapViaUniswapV3Single zero amount", ok ? "ZeroAmountIn" : lastRevertError);
  ...
  try {
    ...
    record("4.12", "INFO", "swapViaUniswapV3Single same token", "no revert (same tokens not rejected)");
  } catch {
  ...
  try {
    const [wethPrice, wethUpdatedAt] = await oracle.getPriceUsd(WETH);
    const [usdcPrice, usdcUpdatedAt] = await oracle.getPriceUsd(USDC);
  ...
  {
    const ok = await staticCallRevert(oracle, "setSource", [USDC, ethers.ZeroHash, 0, 3600n], "OnlyOwner");
    record("5.5", ok ? "PASS" : "WARN", "setSource non-owner reverts", ok ? "OnlyOwner" : lastRevertError);
  ...
  } catch { record("5.7", "WARN", "collateralFactorBps", "failed"); }

  // 5.8 setSource — zero address token validation (should revert)
  {
    const ok = await staticCallRevert(oracle, "setSource", [ethers.ZeroAddress, ethers.ZeroHash, 0, 3600n], "OnlyOwner");
    record("5.8", ok ? "PASS" : "WARN", "setSource zero address token", ok ? "reverted" : lastRevertError);
  ...
  try {
    ...
    record("5.9", price59 > 0n ? "PASS" : "FAIL", "getPriceUsd(USDC) price", `price=${price59}, updatedAt=${updatedAt59}`);
  ...
  try {
    ...
    record("5.10", price510 > 0n && cf510 > 0n ? "PASS" : "FAIL", "getPriceUsd + collateralFactorBps", `price=${price510}, cf=${cf510}`);
  ...
  try {
    const [eC, eS, eB] = await cofhe.encryptInputs([
      Encryptable.uint128(0n), Encryptable.uint128(0n), Encryptable.uint128(0n),
    ]).execute();
    ...
    const r = await tx.wait();
    record("6.1", "PASS", "Composer openPosition (plaintext)", `gas=${r!.gasUsed}`, { tx: tx.hash });
  ...
  record("6.3", okSw ? "PASS" : "WARN", "sweepToken non-owner reverts", okSw ? "OnlyOwner" : lastRevertError);
  ...
  const okAp = await staticCallRevert(executor, "approveToken", [USDC, D.StrategyVault, 0n], "OnlyOwner");
  record("7.1", okAp ? "PASS" : "WARN", "approveToken non-owner reverts", okAp ? "OnlyOwner" : lastRevertError);
  ...
  record("7.2", okWd ? "PASS" : "WARN", "withdrawTokens non-owner reverts", okWd ? "OnlyOwner" : lastRevertError);
  ...
  try {
    ...
    record("8.2", lendable.length > 0 ? "PASS" : "FAIL", "getLendableTokens", `count=${lendable.length}`);
  ...
  try {
    ...
    record("8.3", borrowable.length > 0 ? "PASS" : "FAIL", "getBorrowableTokens", `count=${borrowable.length}`);
  ...
  try {
    ...
    record("8.4", collat.length > 0 ? "PASS" : "FAIL", "getCollateralTokens", `count=${collat.length}`);
  ...
  const tokenInfoStruct = (token: string, lend: boolean, borrow: boolean, collat: boolean, decimals: number) => ({
    token, ltvBps: 5000, liquidationBonusBps: 500, decimals,
    isLendable: lend, isBorrowable: borrow, isCollateral: collat, enabled: true,
    pythPriceId: ethers.ZeroHash, borrowCap: ethers.MaxUint256, supplyCap: ethers.MaxUint256,
  });
  const okReg = await staticCallRevert(tokenReg, "registerToken", [
    tokenInfoStruct(WETH, true, true, true, 18),
  ], "TokenAlreadyRegistered");
  record("8.6", okReg ? "PASS" : "WARN", "registerToken duplicate reverts", okReg ? "TokenAlreadyRegistered" : lastRevertError);
  ...
  record("8.7", okDis ? "PASS" : "WARN", "disableToken non-owner reverts", okDis ? "OnlyOwner" : lastRevertError);
  markCoverage("TokenRegistry", "disableToken", "static_call", "8.7");

  // 8.8 updateTokenConfig — onlyOwner
  const okUpd = await staticCallRevert(tokenReg, "updateTokenConfig", [
    WETH, tokenInfoStruct(WETH, true, true, true, 18),
  ], "OnlyOwner");
  record("8.8", okUpd ? "PASS" : "WARN", "updateTokenConfig non-owner reverts", okUpd ? "OnlyOwner" : lastRevertError);
  ...
  const okPipe = await staticCallRevert(stratExec, "executePipeline", [mockStratId, [{
    actionType: "0x00000000",
    params: "0x",
    encAmount: { ctHash: 0n, securityZone: 0, utype: 0, signature: "0x" },
  }]], "OnlyOwner");
  record("9.1", okPipe ? "PASS" : "WARN", "executePipeline non-owner reverts", okPipe ? "OnlyOwner" : lastRevertError);
  ...
  record("9.2", okReset ? "PASS" : "WARN", "resetCheckpoint non-owner reverts", okReset ? "OnlyOwner" : lastRevertError);
  markCoverage("StrategyExecutor", "resetCheckpoint", "static_call", "9.2");

  // 9.3 sweepToken — onlyOwner
  const okSwStrat = await staticCallRevert(stratExec, "sweepToken", [USDC, tester.address], "OnlyOwner");
  record("9.3", okSwStrat ? "PASS" : "WARN", "sweepToken non-owner reverts", okSwStrat ? "OnlyOwner" : lastRevertError);
  markCoverage("StrategyExecutor", "sweepToken", "static_call", "9.3");

  // 9.4 executePipeline with empty actions (onlyOwner check)
  const okEmpty = await staticCallRevert(stratExec, "executePipeline", [mockStratId, []], "OnlyOwner");
  record("9.4", okEmpty ? "PASS" : "WARN", "executePipeline empty actions reverts", okEmpty ? "OnlyOwner" : lastRevertError);
  ...
  const okMock = await staticCallRevert(stratExec, "executePipeline", [mockStratId, [mockAction]], "OnlyOwner");
  record("9.5", okMock ? "PASS" : "WARN", "executePipeline mock action reverts", okMock ? "OnlyOwner" : lastRevertError);
  ...
  if (!GOV_ADDR) {
    record("10", "WARN", "FheForgeGovernor", "not deployed — address missing from deployment record");
  } else {
    ...
    try {
      ...
      record("10.1", name === "FheForge Governor" ? "PASS" : "FAIL", "name() returns 'FheForge Governor'", name);
    } catch (e: any) { record("10.1", "WARN", "name()", decodeRevert(e)); }
    ...
    } catch (e: any) { record("10.2", "WARN", "proposalThreshold()", decodeRevert(e)); }

    // 10.3 votingDelay / votingPeriod > 0
    try {
      const vd = await gov.votingDelay();
      const vp = await gov.votingPeriod();
      record("10.3", vd > 0n && vp > 0n ? "PASS" : "FAIL", "votingDelay/votingPeriod > 0", `delay=${vd} period=${vp}`);
    } catch (e: any) { record("10.3", "WARN", "votingDelay/votingPeriod", decodeRevert(e)); }
    ...
    try {
      ...
      record("10.5", needs === true ? "PASS" : "WARN", "proposalNeedsQueuing() returns true", `=${needs}`);
    } catch (e: any) { record("10.5", "WARN", "proposalNeedsQueuing()", decodeRevert(e)); }
  ...
  if (!TIMELOCK_ADDR) {
    record("11", "WARN", "FheForgeTimelock", "not deployed — address missing from deployment record");
  } else {
    ...
    try {
      ...
      record("11.1", delay === twoDays ? "PASS" : "WARN", "getMinDelay() == 2 days", `delay=${delay}s`);
    } catch (e: any) { record("11.1", "WARN", "getMinDelay()", decodeRevert(e)); }
    ...
    } catch (e: any) { record("11.2", "WARN", "TIMELOCK_ADMIN_ROLE check", decodeRevert(e)); }
    ...
    } catch (e: any) { record("11.3", "WARN", "PROPOSER_ROLE", decodeRevert(e)); }
    ...
    } catch (e: any) { record("11.4", "WARN", "EXECUTOR_ROLE", decodeRevert(e)); }

    // 11.5 schedule() — non-PROPOSER should revert
    try {
      await timelock.schedule.staticCall(tester.address, 0n, "0x", ethers.ZeroHash, ethers.ZeroHash, 1n);
  ...
  if (!TIMELOCK_ADDR) {
    ...
    } catch (e: any) { record("12.1", "WARN", "PROPOSER_ROLE", decodeRevert(e)); }
    ...
    } catch (e: any) { record("12.2", "WARN", "EXECUTOR_ROLE", decodeRevert(e)); }
    ...
    } catch (e: any) { record("12.3", "WARN", "CANCELLER_ROLE", decodeRevert(e)); }
    ...
    } catch (e: any) { record("12.4", "WARN", "CANCELLER_ROLE deployer", decodeRevert(e)); }
  }
  ...
  if (pythMock) {
    try {
      ...
      record("14.3a", p1.price === 100000000n ? "PASS" : "FAIL",
        "Set Pyth normal price ($1.00)", `price=${p1.price}, expo=${p1.expo}`);
      ...
      record("14.3b", p2.price === 50000000n ? "PASS" : "FAIL",
        "Set Pyth crash price ($0.50)", `price=${p2.price}, expo=${p2.expo}`);
  ...
  if (pythMock && priceFeedId) {
    try {
      ...
      const usdVal = Number(fresh.price) * Math.pow(10, Number(fresh.expo));
      record("14.4", fresh.price === 50000000n ? "PASS" : "FAIL",
  ...
  try {
    const healthy = await pool.isLiquidatable(tester.address, USDC, USDC, liqSupplyAmt, liqBorrowAmt);
    record("14.6", !healthy ? "PASS" : "FAIL", "Position healthy at current prices",
  ...
  try {
    ...
    const unhealthy = await pool.isLiquidatable(tester.address, USDC, USDC, halvedCollateral, liqBorrowAmt);
    record("14.7", unhealthy ? "PASS" : "FAIL", "Position underwater with 50% price drop",
  ...
  // 14.8 — liquidateWithProof: same token (collateralToken == debtToken) → TokenMismatch
  {
    try {
      await pool.connect(deployer).liquidateWithProof.staticCall(
        tester.address, USDC, USDC,
      ...
      const msg = decodeRevert(e);
      const ok = msg.includes("TokenMismatch");
      record("14.8", ok ? "PASS" : "INFO", "liquidateWithProof same-token guard", ok ? "TokenMismatch" : msg);
    }
  }
  ...
  // 14.9 — liquidateWithProof: different tokens (needs valid FHE proofs)
  {
    ...
    try {
      await pool.connect(deployer).liquidateWithProof.staticCall(
        ...
        liqAmt,
        0, "0x",                      // mock debt proof
        0, "0x",                      // mock supply proof
      );
  ...
  // 14.10 — liquidateWithProof self-call guard (CannotSelfLiquidate)
  {
    try {
      await pool.liquidateWithProof.staticCall(
        tester.address, USDC, WETH,
        ethers.parseUnits("500", 6),
        0, "0x",
        0, "0x",
      );
      record("14.10", "FAIL", "liquidateWithProof self-call accepted", "expected CannotSelfLiquidate");
    } catch (e: any) {
      const msg = decodeRevert(e);
      const ok = msg.includes("CannotSelfLiquidate");
      record("14.10", ok ? "PASS" : "INFO", "liquidateWithProof self-call guard",
  ...
  try {
    ...
    record("14.11b", "PASS", "Unshield supplied collateral", `${ethers.formatUnits(liqSupplyAmt, 6)} USDC`);
  ...
  const contractsWithCoverage: Record<string, { covered: number; total: number; functions: CoverageEntry[] }> = {};
  ...
  fs.writeFileSync(path.join(deploymentsDir, "coverage_summary.json"), JSON.stringify(coverageSummary, null, 2));
  ...
  fs.writeFileSync(path.join(deploymentsDir, "uncovered_functions.json"), JSON.stringify(uncoveredFunctions, null, 2));
  ...
  fs.writeFileSync(path.join(deploymentsDir, "failure_clusters.json"), JSON.stringify(failureClusters, null, 2));
  ...
  fs.writeFileSync(path.join(deploymentsDir, "flow_test_results.json"), JSON.stringify(flowResults, null, 2));
  ...
  fs.writeFileSync(path.join(deploymentsDir, "test_reliability_matrix.json"), JSON.stringify(testReliability, null, 2));

  // Public state exposure audit
  const publicStateExposure = [
    { mapping: "positionDepositedAmount", contract: "StrategyVault", visibility: "public", risk: "Exposes exact collateral size bypassing FHE" },
    { mapping: "totalPlainSupply/ totalPlainBorrow", contract: "LendingPool", visibility: "public", risk: "Reveals pool utilization and user debt sizes" },
    { mapping: "getPriceUsd", contract: "PriceOracle", visibility: "public", risk: "On-chain oracle price is public by design" },
  ];
  fs.writeFileSync(path.join(deploymentsDir, "public_state_exposure.json"), JSON.stringify(publicStateExposure, null, 2));
  ...
  fs.writeFileSync(path.join(deploymentsDir, "missed_surface_report.json"), JSON.stringify(missedSurface, null, 2));
  ...
  fs.writeFileSync(path.join(deploymentsDir, "calldata_leak_scan.json"), JSON.stringify(calldataLeakScan, null, 2));

  // Blocker root causes
  const blockerRootCauses: { blocker: string; rootCause: string; classification: string }[] = [];
  fs.writeFileSync(path.join(deploymentsDir, "blocker_root_causes.json"), JSON.stringify(blockerRootCauses, null, 2));
