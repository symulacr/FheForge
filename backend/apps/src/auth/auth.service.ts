...
export class AuthService {
	...
	async generateNonce(walletAddress: string): Promise<{ nonce: string; message: string }> {
		...
		const { error } = await this.supabase.getClient().from("auth_nonces").upsert(
			{
		...
		if (error) {
			this.logger.error(`Failed to store nonce for ${normalizedAddress}: ${error.message}`);
	...
	async login(
		...
		if (selectError || !stored) {
			throw new UnauthorizedException("No nonce found. Request a new nonce first.");
		}
		...
		let user = await this.userService.getUserByWalletAddress(walletAddress).catch(() => null);
	...
	private buildAuthMessage(walletAddress: string, nonce: string): string {
		return ["FheForge Authentication", `Wallet: ${walletAddress}`, `Nonce: ${nonce}`].join("\n");
	}
}
