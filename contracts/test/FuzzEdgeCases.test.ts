...
describe("Fuzz Edge Cases (MC-072)", () => {
	async function deployMinimal() {
		...
		await registry.registerStrategy("Test", ethers.zeroPadValue("0xbeef", 32));
	...
	describe("Boundary values", () => {
		it("reverts on zero-amount borrow when using oracle path", async () => {
			...
			const [encBorrow] = await client.encryptInputs([Encryptable.uint128(0n)]).execute();
	...
	describe("ExecutorContract edge cases", () => {
		it("reverts executing intent with no approval", async () => {
			...
			await expect(router.connect(executor).executeIntent(intentId, ethers.parseEther("5"))).to.be
				.reverted; // ERC20 transferFrom revert
		});
	});
	...
	describe("Fuzzing patterns (manual edge cases)", () => {
		it("handles max uint128 deadline offset", async () => {
			...
			expect(await router.MAX_DEADLINE_OFFSET()).to.equal(2n ** 128n - 1n);
		});
		...
		it("handles roundtrip through zero amounts in constructor validation", async () => {
			...
			const registry = await Registry.deploy(48n * 3600n);
			await registry.waitForDeployment();
			await expect(registry.registerStrategy("", ethers.ZeroHash)).to.be.revertedWithCustomError(
				registry,
				"EmptyName",
			);
		});
