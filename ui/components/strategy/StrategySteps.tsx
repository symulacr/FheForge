	...
	if (steps.length === 0) {
		return (
			<div className={`border border-border bg-card p-6 ${className}`}>
				<div className="text-center text-muted">No strategy steps available</div>
			</div>
		);
	}
		...
		<div className={`border border-border bg-card ${className}`}>
			{showHeader && (
				<div className="border-b border-border p-6 pb-4">
					<h2 className="text-lg font-semibold text-foreground">Strategy Steps</h2>
								...
								<div className="flex-1 min-w-0">
										...
										>
											{step.tokenIn && (
												<div className="flex items-center gap-1">
													<span className="font-medium text-foreground">{step.tokenIn.symbol}</span>
									...
									{!compact && (step.tokenIn?.assetId || step.tokenOut?.assetId) && (
										<div className="mt-1 text-xs text-muted">
											{step.tokenIn?.assetId && <span>In: {step.tokenIn.assetId}</span>}
											{step.tokenIn?.assetId && step.tokenOut?.assetId && (
												<span className="mx-2">•</span>
											)}
											{step.tokenOut?.assetId && <span>Out: {step.tokenOut.assetId}</span>}
										</div>
									)}
								</div>
							</div>
