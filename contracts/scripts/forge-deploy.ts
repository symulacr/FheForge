 ...
 */
import hre, { ethers, network, run, artifacts } from "hardhat";
import * as fs from "fs";
import * as path from "path";
import type { Artifact } from "hardhat/types";
import {
  DeployRecord, ContractInfo,
  elapsed, withRetry,
  Progress, FastVerifier,
...
const ADDRS: Record<number, { pyth: string; weth: string; usdc: string }> = {
  ...
  421614: { pyth: "0x4374e5a8b9C22271E9EB878A2AA31DE97DF15DAF", weth: "0x84BddCAfaccbBDBc0e3F1CAcCDd352EBf5e40A32", usdc: "0x150376EdEbc5AC48771655a61a795d828BeC8Df6" },
...
const ENV_KEYS: Record<string, string> = {
  ...
  SwapRouter: "NEXT_PUBLIC_ROUTER_ADDRESS",
...
async function main() {
  ...
  let rec: DeployRecord = fs.existsSync(depPath)
    ? JSON.parse(fs.readFileSync(depPath, "utf8"))
    : { network: "", chainId, deployer: "", deployedAt: "", mode: "", contracts: {}, deploymentTxs: {}, swapExecutor: "", weth: "" };
  const verified = new Set<string>();
  if (rec.verificationStatus) for (const [k, v] of Object.entries(rec.verificationStatus)) if (v) verified.add(k);
  ...
  const registry = await deploy("StrategyRegistry", "StrategyRegistry", [timing.vaultRotationDelay]);
  const pool = await deploy("LendingPool", "LendingPool");
  const oracle = await deploy("PriceOracle", "PriceOracle", [chainAddrs.pyth, timing.defaultStaleThreshold]);
  ...
  const composer = await deploy("FheForgeComposer", "FheForgeComposer", [
    addrs0.registry, vaultAddr, addrs0.pool, addrs0.router,
  ]);
  const stratExec = await deploy("StrategyExecutor", "StrategyExecutor", [
    addrs0.pool, vaultAddr, addrs0.router,
  ]);
  ...
  if ((await oracle.collateralFactorBps(usdcAddr)) !== 8000) {
    await send("oracle.setCollateral USDC", () =>
      oracle.setCollateralFactor(usdcAddr, 8000, 8500));
  }
  if ((await oracle.collateralFactorBps(wethAddr)) !== 8000) {
    await send("oracle.setCollateral WETH", () =>
      oracle.setCollateralFactor(wethAddr, 8000, 8500));
  }
  ...
  const allContracts: ContractInfo[] = [
    { contractName: "StrategyRegistry", fqn: "contracts/StrategyRegistry.sol:StrategyRegistry", address: addrs0.registry, args: [timing.vaultRotationDelay.toString()], envKey: ENV_KEYS["StrategyRegistry"] },
    { contractName: "LendingPool", fqn: "contracts/LendingPool.sol:LendingPool", address: addrs0.pool, args: [], envKey: ENV_KEYS["LendingPool"] },
    { contractName: "PriceOracle", fqn: "contracts/PriceOracle.sol:PriceOracle", address: addrs0.oracle, args: [chainAddrs.pyth, timing.defaultStaleThreshold.toString()], envKey: ENV_KEYS["PriceOracle"] },
    { contractName: "SwapRouter", fqn: "contracts/SwapRouter.sol:SwapRouter", address: addrs0.router, args: [addrs0.executor, timing.minDeadlineOffset.toString(), timing.maxDeadlineOffset.toString(), timing.executorRotationDelay.toString(), UNISWAP_V3_ROUTER], envKey: ENV_KEYS["SwapRouter"] },
    { contractName: "ExecutorContract", fqn: "contracts/ExecutorContract.sol:ExecutorContract", address: addrs0.executor, args: [], envKey: ENV_KEYS["ExecutorContract"] },
    { contractName: "StrategyVault", fqn: "contracts/StrategyVault.sol:StrategyVault", address: vaultAddr, args: [addrs0.registry], envKey: ENV_KEYS["StrategyVault"] },
    { contractName: "FheForgeComposer", fqn: "contracts/FheForgeComposer.sol:FheForgeComposer", address: composerAddr, args: [addrs0.registry, vaultAddr, addrs0.pool, addrs0.router], envKey: ENV_KEYS["FheForgeComposer"] },
    { contractName: "TokenRegistry", fqn: "contracts/TokenRegistry.sol:TokenRegistry", address: addrs0.tokenRegistry, args: [], envKey: ENV_KEYS["TokenRegistry"] },
    { contractName: "StrategyExecutor", fqn: "contracts/StrategyExecutor.sol:StrategyExecutor", address: stratExecAddr, args: [addrs0.pool, vaultAddr, addrs0.router], envKey: ENV_KEYS["StrategyExecutor"] },
    { contractName: "FheForgeToken", fqn: "contracts/governance/FheForgeToken.sol:FheForgeToken", address: govTokenAddr, args: ["FheForge Token", "FHE"] },
    { contractName: "FheForgeTimelock", fqn: "contracts/governance/FheForgeTimelock.sol:FheForgeTimelock", address: timelockAddr, args: [timing.timelockMinDelay.toString(), deployer.address], envKey: ENV_KEYS["FheForgeTimelock"] },
    { contractName: "FheForgeGovernor", fqn: "contracts/governance/FheForgeGovernor.sol:FheForgeGovernor", address: govAddr, args: [govTokenAddr, timelockAddr, timing.governorVotingDelay.toString(), timing.governorVotingPeriod.toString(), timing.governorQuorumBps.toString()], envKey: ENV_KEYS["FheForgeGovernor"] },
  ];
  ...
  if (isBench) {
    ...
    console.log(`  Verified: ${Object.values(rec.verificationStatus ?? {}).filter(Boolean).length}`);
