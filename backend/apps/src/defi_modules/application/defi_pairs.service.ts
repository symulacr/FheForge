...
export class DefiPairsService {
  ...
  private estimateSupply(
    tokenInId: string,
    amountIn: number,
  ): EstimateDefiPairResponseDto {
  ...
  private async estimateBorrow(
    ...
    if (!borrowApyBps)
      throw new Error('BORROW_APY_BPS env variable is not set');
    ...
    const maxBorrowAmount =
      collateralAmount * (ltv - ltv * 0.1) * spotPrice * 0.99;
