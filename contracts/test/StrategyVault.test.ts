...
describe("StrategyVault", () => {
	async function deployAll() {
		...
		await registry.registerStrategy("Test Strategy", ethers.zeroPadValue("0xdeadbeef", 32));
	...
	async function encryptOpenPosition(userClient: any, collateralWei: bigint) {
		const [eCollateral] = await userClient
			.encryptInputs([Encryptable.uint128(collateralWei)])
			.execute();
		return {
	...
	async function setupPositionAcl(vault: any, user: any, ctHashBigInt: bigint): Promise<void> {
	...
	async function openPosition(
		...
		const tx = await vault
			.connect(user)
			.openPosition(await token.getAddress(), collateralWei, eCollateral, 1n, user.address);
	...
	it("openPosition adds a user position", async () => {
		...
		const { eCollateral, ctHashBigInt } = await encryptOpenPosition(userClient, collateralWei);
	...
	it("collateral plaintext matches via mock expectPlaintext", async () => {
		...
		const _positionId = await openPosition(vault, token, user, eCollateral, collateralWei);
	...
	it("second user cannot read first user collateral", async () => {
		...
		await expect(vault.connect(user2).getCollateral(positionId)).to.not.be.reverted;
