...
function getApy(
	...
	switch (type) {
		case "SUPPLY":
			return toNumber((estimate as DefiEstimate)?.supply_apy ?? (estimate as DefiEstimate)?.apy, 0);

		case "BORROW":
			return toNumber((estimate as DefiEstimate)?.borrow_apy ?? (estimate as DefiEstimate)?.apy, 0);
		default:
			return toNumber((estimate as DefiEstimate)?.apy, 0);
	}
}

export function formatAmount(value?: number | string, fractionDigits = 2): string {
...
export function resolveDefiOperationType(data: DefiNodeData): DefiOperationType | undefined {
	...
	const estimateType = isValidType(estimate?.operation_type) ? estimate.operation_type : undefined;
...
export function normalizeDefiNodeData(data: DefiNodeData): NormalizedDefiNodeData {
	...
	const tokenInSymbol =
		config?.tokenInSymbol || data?.action?.defi_pairs?.[0]?.token_in?.name || "-";

	const tokenOutSymbol =
		config?.tokenOutSymbol || data?.action?.defi_pairs?.[0]?.token_out?.name || "-";

	const amount = toNumber(config?.amount, NaN) || toNumber(estimate?.amount_in, 0);

	const amountOut = toNumber(config?.amountOut, NaN) || toNumber(estimate?.amount_out, 0);

	const slippage = toNumber(config?.slippage, NaN) || toNumber(estimate?.slippage, 0);
	...
	const title =
		data?.title || data?.label || data?.action?.name || data?.module?.name || "DeFi Action";

	const protocolName = data?.module?.protocol || data?.module?.name || "Unknown Protocol";
