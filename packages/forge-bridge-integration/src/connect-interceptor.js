...
function checkAndSwitchNetwork(chainId) {
  ...
  return getBridge()
    ...
    .catch((switchErr) => {
      ...
      throw new Error('Network mismatch. Please switch to Arbitrum Sepolia (chain ID 421614) in your wallet.');
    });
}
...
function executePermitGrant() {
  return getBridge()
    ...
    .then((permitResult) => ({
      ...
      secondsLeft: permitResult && permitResult.secondsLeft != null ? permitResult.secondsLeft : 900,
    }));
}
...
function startConnectFlow(connectorId) {
  ...
  if (!connectorId && ss) {
    try { connectorId = ss.getItem(SESSION_STORAGE_CONNECTOR_KEY) || 'metaMask'; } catch { connectorId = 'metaMask'; }
  }
  if (!connectorId) connectorId = 'metaMask';

  // Persist wallet choice
  if (ss) {
    try { ss.setItem(SESSION_STORAGE_CONNECTOR_KEY, connectorId); } catch {
