...
export class StrategyConstraintsService {
	...
	constructor(
		...
		readonly _defiTokenService: DefiTokenService,
		private readonly configService: ConfigService,
	) {
		this.wethTokenId = this.configService.get<string>("TOKEN_WETH_ID", "weth-token-id");
		this.usdcTokenId = this.configService.get<string>("TOKEN_USDC_ID", "usdc-token-id");
		this.usdtTokenId = this.configService.get<string>("TOKEN_USDT_ID", "usdt-token-id");
	}
	...
	async getStrategyConstraints(
		...
		const operationConstraints = this.analyzeOperationConstraints(userIntent, additionalContext);
	...
	private getTokenBySymbol(symbol: string): {
		...
		const tokenMap: Record<string, { id: string; symbol: string; assetId: string }> = {
			WETH: {
				id: this.wethTokenId,
				symbol: "WETH",
	...
	private analyzeOperationConstraints(
		...
		if (context.includes("max") && context.includes("leverage")) {
			...
			if (leverageMatch) {
				maxLeverage = parseInt(leverageMatch[1], 10);
			}
		}
